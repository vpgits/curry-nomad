...
// shot is paired (by index) with its text-to-image prompt — info the brief card only summarises as
...
export function StoryboardFilmstrip({
  ...
  const promptFor = (index: number) =>
    shot_prompts.find((p) => p.index === index)?.image_prompt ?? "";
    ...
    <Card className="gap-4">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          Storyboard · {shots.length} images
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          {shots.map((shot) => (
            ...
            >
              <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                ...
                {shot.on_screen_text && (
                  <span className="truncate italic">{shot.on_screen_text}</span>
                )}
              </div>
              <div className="aspect-square rounded-md border border-dashed bg-gradient-to-br from-muted to-accent/30" />
